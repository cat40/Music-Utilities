...
from pygame import mixer #this is a little slow, see if another dedicate module would be faster
import os
#os.environ['LIBROSA_CACHE_DIR'] = 'E:\\librosa_cache' #enable librosa caching
...
class LibrosaObject(object):
    ...
    '''
    def __init__(self, fname, usecache=True, cachePath='.\\cache\\', cachePathOverride=None): #change cache to False once testing is completed
        #consider using trim to remove silence on both ends before processing, but remember to add silence back before exporting information (or just play the trimmed file instead)
        #also find a way to cut applause and other noise. (maybe percussion splitting?)
        self.fname = fname
        #load the file
        self.waveform, self.samplingrate = librosa.load(fname)#, sr=None) #sr=None sets it to the file's sampling rate (probably 44100hz). default is 22050
        self.usecache = usecache
        if self.usecache:
            self._cachePath = os.path.join(cachePath, "'"+os.path.basename(self.fname)+"'") if cachePathOverride is None else cachePathOverride
            self.cache = Cache(self._cachePath)
##            try: os.mkdir(self.cachePath)
##            except OSError: pass
    ...
    def getTempoAtPoint(self, time, frame=5):
        ...
        releventTimes = filter(lambda x : x >= time-frame and x <= time+frame, list(self.beattimes))
    ...
    def sliceAudio(self, split=10):
        ...
        duration = librosa.core.time_to_samples(librosa.core.get_duration(y=self.waveform, sr=self.samplingrate), sr=self.samplingrate)
        for t in xrange(0, int(duration+0.5), split):
            ...
            #librosa.output.write_wav(os.path.splitext(self.fname)[0] + '_%s.wav' % t, self.waveform[t:min(t+split, duration)], self.samplingrate)
    ...
    def getOnsets(self, debug=False):
        if self.usecache:
            try: return self.cache.load('onsets.npy')
        ...
        for i, y in enumerate(self.splitByFifths(y=harmonic)):
            ...
            if debug:
                clicks = librosa.core.clicks(frames=librosa.core.samples_to_frames(onsets_i), sr=self.samplingrate, click_freq=220)
                librosa.output.write_wav(os.path.join('..\\tests\\results', os.path.splitext(os.path.basename(self.fname))[0]+'_onsets_octave_%s.wav' % i), clicks, self.samplingrate)
        if debug:
            librosa.output.write_wav(os.path.join('..\\tests\\results', os.path.splitext(os.path.basename(self.fname))[0]+'_percussive.wav'), percussive, self.samplingrate)
            librosa.output.write_wav(os.path.join('..\\tests\\results', os.path.splitext(os.path.basename(self.fname))[0]+'_harmonic.wav'), harmonic, self.samplingrate)
            clicks = librosa.core.clicks(frames=librosa.core.samples_to_frames(onsetsP), sr=self.samplingrate, click_freq=220)
            librosa.output.write_wav(os.path.join('..\\tests\\results', os.path.splitext(os.path.basename(self.fname))[0]+'_onsets_p.wav'), clicks, self.samplingrate)
        if self.usecache:
...
##                                  (onsets, 'onsets'))
    ...
    def getOnsets2(self, debug=False):
        if self.usecache:
            try: return self.cache.load('onsets2')
            except :#cModule.CacheNotFoundError:
                pass
    ...
    def getPitch(self, y, i, j, x, fmin=16, fmax=4000):
        ...
        cached = False
        if self.usecache:
            try: r = self.cache.load('autocorrelation-%s-%s-%s.npy' % (i, j, x))
        ...
        if not cached:
            ...
            if self.usecache:
                self.cache.write(r, 'autocorrelation-%s-%s-%s' % (i, j, x)) #this causes the re-use of autocorrelations. need to fix. Perhaps try autocorrelating the whole thing at once
    ...
    def getPitchCheap(y, sr, fmin=16, fmax=4000):
        ...
        i = numpy.unravel_index(magnitudes.argmax(), magnitudes.shape) #unravel_index transforms the bizzare integer returned by argmax into a tuple index
        return pitches[i]
    ...
    def getNotes(self):
        ...
        for x, (onsets, y) in enumerate(self.getOnsets(debug=True)):
            ...
            for i, j in zip([0]+list(onsets), list(onsets)+[len(y)]): #numpy.dstack(([0]+onsets, onsets+[len(y)]))[0]:#[None]):
                print 'in i/j loop', \
    ...
    def outputNotes(self):
        ...
        librosa.output.write_wav(os.path.join('..\\tests\\results', os.path.splitext(os.path.basename(self.fname))[0]+'_notes.wav'), waveform, self.samplingrate)
    ...
    def testInvert(self):
        ...
        librosa.output.write_wav(os.path.join('..\\tests\\results', os.path.splitext(os.path.basename(self.fname))[0]+'_test_invert.wav'), yInv, self.samplingrate)
...
##        print freqs[1, numpy.argmax(freqs[1])]*self.samplingrate
        ...
        for t, (mag, pitch) in enumerate(zip(list(m), list(p))):#xrange(librosa.core.samples_to_frames(len(self.waveform))):
            i = numpy.argmax(mag)
            yield pitch[i], mag[i]
    ...
    def writeNotes(self):
        ...
        for i, (pitch, mag) in enumerate(self.pitchCurve()):
            ...
            #print i, pitch, mag
            waveform = numpy.concatenate([waveform, [sine(x)  for x in xrange(librosa.core.frames_to_samples(1))]])

            #print 'bin done'
        librosa.output.write_wav(os.path.join('..\\tests\\results', os.path.splitext(os.path.basename(self.fname))[0]+'_notes2.wav'), waveform, self.samplingrate)

    def magCurve(self):
        phrase = librosa.core.frames_to_samples(1, self.samplingrate)
        for t in xrange(1, librosa.core.samples_to_frames(len(self.waveform))):
            pass
...
##
    ...
    @staticmethod
    def helper_butter(sr, fmin=0, fmax=None, order=6, output='sos'): #possible candidate for caching, but would need to be made non-static
        nyquist = sr/2
    ...
    def condenseOnsets(cls, onsets):
        ...
        for i, onset in iteronsets:
            if not cls.closeTo(onset, onsets[i-1]): #this will remove all onsets that are chained just close enough to each other, may not be desireable
    ...
    '''
    def splitByOctave(self, octavesPerBin=1, y=None, octmin=6, octmax=9): #another candiate for caching, would need to be made a non generator object, or something similar (possibly dynamically write a list?)
        #will ignore remander if ocatves%octavesPerBin
        assert octmin >= 1
        assert octmax <= int(math.log(self.samplingrate/(2*librosa.core.note_to_hz('c0')), 2))
        if y is None: y = self.waveform
        for octave in xrange(octmin, octmax+1, octavesPerBin):
...
if __name__ == '__main__':
    import sys
    sys.exit()
    #test = librosaObject('cmajor.wav')
    #test = librosaObject('Firebird Good.mp3')
    #test = librosaObject('tests\\15 Romantic Flight.mp3')
    test = librosaObject('tests\\Kongo Themes.mp3')
    ...
    test.outputNotes()
    #test.getNotes()
    #test = librosaObject('tests\\sine220-440.wav')
    #test.chromogram()
    #test.getOnsets2()
##    for i, y in enumerate(test.splitByOctave()):
##        librosa.output.write_wav(os.path.join('tests\\results', os.path.splitext(os.path.basename(test.fname))[0]+'_octave'+str(i)+'.wav'),
##                                  y, test.samplingrate)

    
##    for f in ('tests\\pitchdown.wav', 'tests\\pitchup.wav', 'tests\\tempodown.wav',
##              'tests\\tempoup.wav', 'tests\\volumedown.wav', 'tests\\volumeup.wav'):
##        test = librosaObject(f)
##        test.getOnsets()
##    c = librosaObject(librosa.util.example_audio_file())
##    print c.getTempo()
##    print
##    print 60/c.getTempoAtPoint(15)
##    print
##    print 60/c.getTempoAtPoint(40)
##    a = librosaObject('Firebird Good.mp3')
##    print a.getTempo()
##    print
##    print a.getTempoAtPoint(15)
##    print
##    print a.getTempoAtPoint(200)
##    print
##    b = librosaObject('Baccinale.mp3')
##    #b.plotBeats()
##    print 60/b.getTempo()
##    print b.getTempoAtPoint(25)
##    print b.getTempoAtPoint(100)
