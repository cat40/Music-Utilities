...
class Xml_node:
    ...
    def get_maybe_exist_typed_child (self, klass):
        ...
        if len (cn)==0:
            ...
            raise Exception("More than 1 child", klass)

    def get_unique_typed_child (self, klass):
        cn = self.get_typed_children(klass)
        if len (cn) != 1:
            ly.error (self.__dict__)
            raise Exception('Child is not unique for', (klass, 'found', cn))
    ...
    def extract_voices (part):
        ...
        for n in elements:
            ...
            if isinstance (n, Direction):
                ...
                if staff_id:
                    dir_voices = staff_to_voice_dict.get (staff_id, list(voices.keys ()))
                else:
                    dir_voices = list(voices.keys ())
                for v in dir_voices:
                    voices[v].add_element (n)
                continue
